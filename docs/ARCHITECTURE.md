...
| `type` | Ecto.Enum, NOT NULL | identity kind (`:email`, `:phone`, plus social provider atoms) |
| `value` | citext, NOT NULL | the email or phone string |
...
| `id` | bigint PK | auto-generated surrogate key |
| `user_id` | bigint FK -> users, NOT NULL, cascade | owning user |
| `token` | binary, NOT NULL, unique | opaque random bytes; lives in the cookie |
...
| `token` | string, NOT NULL, unique | SHA-256 hash of the raw token |
| `value` | string, nullable | flow-specific payload (e.g., the new email for `:email_change`) |
